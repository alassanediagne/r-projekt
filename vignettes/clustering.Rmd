...
K-Medoids, similar to k-means is an iterative partitional algorithm, but is NP-hard and offers robustness against outliers.

### Implementation

The algorithm takes as input a data matrix, where each row represents a data point, and an integer num_cluster that specifies the desired number of clusters. Optionally, you can set a maximum number of iterations via the max_iter parameter, which has a default value of 100.  Each data point is then assigned to the closest medoid based on Euclidean distance. For each cluster, the algorithm selects the point within the cluster that minimizes the total distance to all other points in that cluster. This point becomes the new medoid. The algorithm iterates these steps until the medoids stop changing, meaning the algorithm has converged.

Let’s go back to the iris dataset and focus on clustering based on petal size. First, we prepare the data by selecting the petal length and width:
...
Now, we perform k_medoids clustering with 3 clusters, which is most suitable for the iris dataset:
